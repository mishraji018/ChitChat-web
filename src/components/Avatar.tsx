...
const UserAvatar = ({ name, color, size = 'md', isOnline, image, className }: AvatarProps) => {
  const initials = name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
  return (
    <div className={`relative inline-flex shrink-0 ${className || ''}`}>
      {image ? (
        <img src={image} alt={name} className={`${sizes[size]} rounded-full object-cover shadow-sm`} />
      ) : (
        <div className={`${sizes[size]} rounded-full flex items-center justify-center font-bold shadow-sm`} style={{ backgroundColor: color, color: '#fff' }}>
          {initials}
        </div>
      )}
      {isOnline !== undefined && (
